...
const PropertyAnalyzer = () => {
  ...
  const calculateScenarios = () => {
    ...
    // Calculate average cashflow by down payment percentage
    const cashflowByDP = downPaymentPercentages.map(dp => {
      const scenariosWithDP = allScenarios.filter(s => s.downPaymentPercentage === dp);
      const avgMonthlyCashflow = scenariosWithDP.reduce((sum, s) => sum + s.monthlyCashflow, 0) / scenariosWithDP.length;
      const positiveCashflowCount = scenariosWithDP.filter(s => s.monthlyCashflow > 0).length;
      const positiveCashflowPercentage = (positiveCashflowCount / scenariosWithDP.length) * 100;
    ...
    setCashflowByDownPayment(cashflowByDP);
    
    // Calculate ROI distribution by cap rate
    const roiByCapRateData = capRates.map(cr => {
      const scenariosWithCapRate = allScenarios.filter(s => s.capRate === cr);
      const minROI = Math.min(...scenariosWithCapRate.map(s => s.annualROI));
      const maxROI = Math.max(...scenariosWithCapRate.map(s => s.annualROI));
      const avgROI = scenariosWithCapRate.reduce((sum, s) => sum + s.annualROI, 0) / scenariosWithCapRate.length;
    ...
    // Calculate ROI distribution by purchase price
    const roiByPriceData = purchasePrices.map(pp => {
      const scenariosWithPrice = allScenarios.filter(s => s.purchasePrice === pp);
      const avgROI = scenariosWithPrice.reduce((sum, s) => sum + s.annualROI, 0) / scenariosWithPrice.length;
      const positiveCashflowCount = scenariosWithPrice.filter(s => s.monthlyCashflow > 0).length;
      const positiveCashflowPercentage = (positiveCashflowCount / scenariosWithPrice.length) * 100;
      
      return {
        purchasePrice: pp / 1000, // Convert to K
        avgROI,
        positiveCashflowPercentage
      };
    });
    setRoiByPurchasePrice(roiByPriceData);
  };
